(ns clj-simple-chart.bp.diagrams.wind-consumption-per-capita-top
...
(def data (->> bpdata/most-recent-data-countries
               (filter :wind_consumption_twh)
               (filter :population)
               (csv/keep-columns [:wind_consumption_twh :population :country :country_code])
               (mapv #(assoc % :total (/ (* units/billion (:wind_consumption_twh %))
                                         (/ (:population %) 1))))
               (csv/drop-columns [:wind_consumption_twh])
               (filter #(pos? (:total %)))
               (sort-by :total)
               (take-last 20)))
...
(def header (opentype/stack
              {}
              [{:text (str "Top " (count data) " Wind Electricity Consumers") :font "Roboto Black" :font-size 18}
...
(defn render-self []
  (render "./img/bp-svg/wind-consumption-per-capita-top.svg" "./img/bp-png/wind-consumption-per-capita-top.png" (diagram)))
