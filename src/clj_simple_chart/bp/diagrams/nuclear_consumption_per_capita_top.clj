(ns clj-simple-chart.bp.diagrams.nuclear-consumption-per-capita-top
...
(def data (->> bpdata/most-recent-data-countries
               ...
               (filter :population)
               (csv/keep-columns [:nuclear_consumption_twh :population :country :country_code])
               (mapv #(assoc % :total (/ (* units/million (:nuclear_consumption_twh %))
                                         (/ (:population %) 1))))
               (csv/drop-columns [:nuclear_consumption_twh])
               (filter #(pos? (:total %)))
               (sort-by :total)
               (take-last 20)))
...
(def header (opentype/stack
              {}
              [{:text (str "Top " (count data) " Nuclear Electricity Consumers") :font "Roboto Black" :font-size 17}
...
(defn render-self []
  (render "./img/bp-svg/nuclear-consumption-per-capita-top.svg" "./img/bp-png/nuclear-consumption-per-capita-top.png" (diagram)))
