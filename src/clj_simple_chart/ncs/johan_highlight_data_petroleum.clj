(ns clj-simple-chart.ncs.johan-highlight-data-petroleum
...
(test/is (= top-11-players-names
            ["EKOFISK" "STATFJORD" "GULLFAKS" "SNORRE" "OSEBERG" "TROLL" "ÅSGARD" "SNØHVIT" "ORMEN LANGE"
...
(def cats (vec (flatten [(drop-last 2 top-11-players-names)
                         "OTHERS_PRE_1990"
                         "OTHERS_POSTE_1990"
                         "OTHERS_POSTE_2000"
                         "JOHAN SVERDRUP"
...
(def flat-data-petroleum
  (for [yr (range discovery/start-year (inc discovery/stop-year))]
    (reduce (fn [o [v flds]] (assoc o v (remaining-at-time flds yr :petroleum)))
...
(csv/write-csv-format "data/ncs/discovery/johan-highlight-petroleum.csv"
                      {:columns (vec (cons "YEAR" (mapv first cat-name-and-flds)))
                       :data    flat-data-petroleum
